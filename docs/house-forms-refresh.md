...
3. **본문 위치 선언 `"body"`(사람).** 서식엔 `{{body}}` 마커가 없어서 `write` 가
   본문을 **마지막 섹션(판권지·참고문헌)에 붙여** 엉뚱한 쪽 날개 밑에 들어간다(실측:
   판권지에 본문 100쪽). 마커도 문서에 심지 말고 **config 로 선언**하면(어느 섹션이
   본문인지 렌더로 판정) 굽는 단계에서 사본에 `{{body}}` 문단을 삽입한다:

   ```json
   // overrides/정책과제_서식.hwpx.json — 보고서 4종 공통(section4=본문)
   { "body": { "section": 4, "position": "start" } }
   ```

   `"strip": true` 를 주면 그 섹션의 **예시 문단을 걷어내고** `[빈 opener][{{body}}]`
   만 남긴다(표지·판권·목차·들어가며 등 다른 섹션은 무손상). 서식의 본문·목차·들어가며
   예시는 전부 "형식 시연용 껍데기"(연구자는 바탕글에 수동 서식으로 편집, 회사는 겉모양만
   봄)라 걷어내도 잃을 실데이터가 없다. `styles` 없이 `body` 만 두면 스타일 자동추론 유지.
   삽입/스트립은 `insert_body_marker()` 가 컨테이너 보존으로 처리. 보고서 4종은
   `section4`(strip), 이력카드(설문폼)·브리프(본문 다중섹션)는 미선언.
